...
#define _INTERVAL_SERIAL  250 // serial interval (unit: ms)
...
float volt_to_distance(unsigned int a_value) 
{
  ...
  //return (636.0 + (-2.25 * a_value) + (2.01E-3 * (a_value * a_value)));
  return 612.0 + (-2.67 * a_value) + (4.19E-03 * a_value * a_value) + (-2.4E-06 * a_value * a_value * a_value);
}
