...
#define _EMA_ALPHA 0.125    // EMA weight of new sample (range: 0 to 1)
...
#define _DUTY_MIN 553 // servo full clockwise position (0 degree) ~ 553
#define _DUTY_NEU 1476 // servo neutral position (90 degree) ~ 1476 ~ 1481
#define _DUTY_MAX 2399 // servo full counterclockwise position (180 degree) 2399 ~
...
void loop() {
  ...
  if(dist_ema <= _TARGET_LOW) {
    ...
    //myservo.writeMicroseconds(_DUTY_NEU);
    myservo.writeMicroseconds(((_DUTY_MAX - _DUTY_MIN) / (float)(_TARGET_HIGH - _TARGET_LOW)) * (dist_ema - _TARGET_LOW) + _DUTY_MIN);
  } else {
