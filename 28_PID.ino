...
#define _EMA_ALPHA 0.50    // EMA weight of new sample (range: 0 to 1)
                          // Setting EMA to 1 effectively disables EMA filter

// Servo adjustment - Set _DUTY_MAX, _NEU, _MIN with your own numbers
#define _DUTY_NEU 1250
#define _DUTY_MAX (_DUTY_NEU + 500)
#define _DUTY_MIN (_DUTY_NEU - 500)

#define _SERVO_ANGLE_DIFF    90  // Replace with |D - E| degree
#define _SERVO_SPEED       1000  // servo speed 

// PID parameters
#define _KP    8.000000   // proportional gain
#define _KD  170.000000   // derivative gain
#define _KI    0.700000  // integral gain
//#define _KI (dist_target <= 155 ? 0.75 : 0.25)
...
////////////////////////////////////////////////////////////////////////
    ...
    if( abs(dist_target - dist_ema) <= 50 ) {
        if( abs(dist_target - dist_ema) > 3 )
            iterm += _KI * error_current;
    } else iterm = 0.0;

    /*
    if(prev_toggle_cnt != toggle_cnt) {
        iterm = 0.0;
        //prev_toggle_cnt = toggle_cnt;
        //toggle_cnt = -2;
    }
    */
    //toggle_cnt %= 4; // for infinity loop

    if( (dist_prev >= dist_target && dist_ema <= dist_target)
    ||  (dist_prev <= dist_target && dist_ema >= dist_target)) {
        iterm = 0.0;
    }
    dist_prev = dist_ema;
  ...
  if (event_serial) {
    ...
    if (0) { // use for debugging with serial plotter
      ...
      Serial.print(",ERR_ave(10X):");        Serial.print(error_sum / (float) error_cnt * 10);
      Serial.print(",iterm:");   Serial.println(min(max(iterm / 10, -155), 155) + 155);
    }
...
float volt_to_distance(unsigned int a_value) 
{
  ...
  return 882.0 + (-4.22 * a_value) + (7.28E-03 * a_value * a_value) + (-4.48E-06 * a_value * a_value * a_value);
}
